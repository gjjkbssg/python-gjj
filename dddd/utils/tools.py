...
def build_extracted_features_dataset(folder_path, feature_name, frequency_band):
    ...
    label_dict = {}
    try:
        all_mat_file = os.walk(folder_path) # 生成器 os.walk 返回一个三元组 (dirpath, dirnames, filenames)
            ...
            for file_name in file_list:
                ...
                if file_name not in skip_set:
                    all_features_dict = scio.loadmat(os.path.join(folder_path, file_name),
                                                     verify_compressed_data_integrity=False) # 读取 mat 文件
                    subject_name = file_name.split('.')[0] # 文件名,gjj1.mat就会被分割成gjj1
                    feature_vector_trial_dict = {} # 特征向量字典
                    label_trial_dict = {} # 标签字典
                    for trials in range(1, 16): # 15个trials
                        feature_vector_list = [] # 特征向量列表
                        label_list = [] # 标签列表
                        cur_feature = all_features_dict[feature_name + str(trials)] # 特征名+trials，如de_LDS1 什么意思？ 
                        cur_feature = np.asarray(cur_feature[:, :,frequency_idx]).T  # 转置后，维度为 N * 62,frequency_idx起什么作用？ 
                        feature_vector_list.extend(_ for _ in cur_feature) # 将特征向量列表扩展,怎么扩展的？ 62 * 235
                        for _ in range(len(cur_feature)):  # 235
                            label_list.append(labels[trials - 1]) # 截取片段对应的 label，-1, 0, 1
                        feature_vector_trial_dict[str(trials)] = feature_vector_list # 特征向量字典
                        label_trial_dict[str(trials)] = label_list # 标签字典
                    feature_vector_dict[subject_name] = feature_vector_trial_dict # 特征向量字典
                    label_dict[subject_name] = label_trial_dict # 标签字典
                else:
                    continue
...
if __name__ == '__main__':
     # 测试代码
    extracted_features_folder_path = 'F:\SEED\SEED_EEG\ExtractedFeatures/'
    a,b=build_extracted_features_dataset(extracted_features_folder_path, 'de_LDS', 'gamma')
