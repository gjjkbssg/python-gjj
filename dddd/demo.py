...
folder_path = 'F:\SEED\SEED_EEG\ExtractedFeatures/'
...
def all_data_svm(folder_path):
    ...
    '''
    # 样本加载
    de_LDS_feature_dict, de_LDS_label_dict = build_extracted_features_dataset(folder_path, 'psd_LDS', 'delta') # 读取 de_LDS 特征
    de_LDS_feature_list = [] 
    de_LDS_label_list = []
...
def paper_svm(folder_path):
    ...
    accuracy_list = []
    # 样本加载
    de_LDS_feature_dict, de_LDS_label_dict = build_extracted_features_dataset(folder_path, 'psd_LDS', 'gamma')
    accuracy = 0
    for key in de_LDS_feature_dict.keys():
        ...
        svc_classifier.fit(train_feature, train_label) # 训练
        pred_label = svc_classifier.predict(test_feature) # 预测
        print(confusion_matrix(test_label, pred_label))
        print(classification_report(test_label, pred_label))
        cur_accuracy = svc_classifier.score(test_feature, test_label)
        accuracy += cur_accuracy
        print('当前 experiment 的 accuracy 为：{}'.format(cur_accuracy))
        #把每个 experiment 的 accuracy 存储起来
...
a = paper_svm(folder_path) # 计算每个 experiment 上的 accuracy
